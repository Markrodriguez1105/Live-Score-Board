...
export const Spotlight: React.FC<SpotlightProps> = ({ candidate, onNext, onPrev, currentIndex, totalCandidates, controlsVisible = true }) => {
    ...
    useEffect(() => {
        ...
        const animate = (time: number) => {
            ...
            const value = progress * (endValueRef.current - startValueRef.current) + startValueRef.current;
        ...
        <div className="spotlight-container relative w-full h-full overflow-hidden bg-black text-white">
            ...
            <div className="absolute inset-x-0 bottom-0 md:inset-y-0 md:right-0 md:left-auto h-2/3 md:h-full md:w-1/2 flex flex-col justify-end md:justify-center items-center px-6 md:px-12 z-10 space-y-4 md:space-y-8 pb-20 md:pb-0">
                <div className="text-center space-y-2 animate-fade-in-up">
                    ...
                    <p className="text-pageant-gold text-xl md:text-2xl tracking-[0.3em] md:tracking-[0.5em] uppercase font-light">Candidate</p>
                </div>

                {/* Scores Grid */}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 w-full max-w-2xl mt-4 md:mt-8">
                    {candidate.scores.map((score, i) => (
                        <div key={i} className="flex flex-col items-center p-3 md:p-6 rounded-xl md:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-2xl">
                            <span className="text-[10px] md:text-xs text-pageant-gold uppercase tracking-widest mb-1 md:mb-2 font-bold">Judge {i + 1}</span>
                            <span className="text-3xl md:text-6xl font-bold font-mono text-white flex items-baseline">
                                {score.toFixed(0)}
                                <span className="text-pageant-gold text-2xl md:text-5xl ml-1 md:ml-2">%</span>
                            </span>
                        </div>
                    ))}
                </div>
            ...
            {controlsVisible && (
                <div className="absolute bottom-8 right-12 flex space-x-4 z-50 opacity-50 hover:opacity-100 transition-opacity">
                    <button type="button" onClick={onPrev} aria-label="Previous Candidate" className="p-3 bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-md border border-white/20">
                    ...
                    <button type="button" onClick={onNext} aria-label="Next Candidate" className="p-3 bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-md border border-white/20">
