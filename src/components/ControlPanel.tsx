...
interface ControlPanelProps {
    ...
    loading: boolean;
    error: string | null;
    refresh: () => Promise<Candidate[]>;
    isIdle: boolean;
    toggleIdle: (state: boolean) => void;
}

export const ControlPanel: React.FC<ControlPanelProps> = ({ candidates, currentIndex, setIndex, loading, error, refresh, isIdle, toggleIdle }) => {
    ...
    const handleCandidateClick = async (index: number) => {
        ...
        const newCandidates = await refresh();
        setIndex(index, newCandidates);
    };

    if (loading && candidates.length === 0) return <div className="p-8 text-white">Loading data...</div>;
    if (error && candidates.length === 0) return <div className="p-8 text-red-500">Error: {error}</div>;

    const currentCandidate = candidates[currentIndex];

    // Filter Logic
    const categories = ['All', ...Array.from(new Set(candidates.map(c => c.category || 'General')))];

    const filteredCandidates = candidates
        .map((c, i) => ({ ...c, originalIndex: i }))
        .filter(c => selectedCategory === 'All' || (c.category || 'General') === selectedCategory);
            ...
            <div className="w-full md:w-1/3 border-t md:border-t-0 md:border-r border-gray-800 flex flex-col h-1/2 md:h-full bg-gray-900">
                <div className="p-4 border-b border-gray-800 bg-gray-900 sticky top-0 z-10 space-y-3">
                    ...
                    <div className="flex justify-between items-center text-xs text-gray-500">
                        <span>{filteredCandidates.length} / {candidates.length} Shown</span>
                    </div>
                    ...
                    <button onClick={() => refresh()} className="text-xs bg-gray-800 p-3 rounded hover:bg-gray-700 w-full flex justify-center items-center space-x-2 touch-manipulation">
                ...
                <div className="overflow-y-auto flex-1 p-2 md:p-0">
                    {filteredCandidates.map((c) => (
                        <div
                            ...
                            `}
                        >
                            <div className="font-mono text-sm opacity-50 w-6">{c.originalIndex + 1}</div>
                            <img src={c.photoUrl} className="w-10 h-10 rounded-full object-cover bg-gray-700" alt="" />
                            <div className="flex-1 min-w-0">
                                <div className="font-bold truncate">{c.name}</div>
                                {c.category && <div className="text-xs opacity-70 uppercase truncate">{c.category}</div>}
                            </div>
                            {currentIndex === c.originalIndex && <div className="text-xs bg-pageant-gold text-black px-2 py-1 rounded font-bold">LIVE</div>}
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col h-1/2 md:h-full overflow-hidden bg-black relative">
                    ...
                    {currentCandidate ? (
                        <>
                            <div
                                ...
                                style={{ backgroundImage: `url("${currentCandidate.photoUrl}")` }}
                            />
                            <div className="relative z-10 text-center space-y-2 md:space-y-4 p-4">
                                <img title={currentCandidate.name} src={currentCandidate.photoUrl} className="w-24 h-24 md:w-48 md:h-48 rounded-full border-4 border-pageant-gold mx-auto object-cover shadow-xl" />
                                <div>
                                    <div className="text-pageant-gold uppercase tracking-widest text-xs md:text-sm font-bold mb-1">{currentCandidate.category || 'Candidate'}</div>
                                    <h1 className="text-2xl md:text-5xl font-bold truncate px-2">{currentCandidate.name}</h1>
                                </div>
                                <div className="text-lg md:text-xl font-mono opacity-70">
                                    Current Score: {(currentCandidate.totalPercentage || 0).toFixed(2)}%
                                </div>
                            </div>
                        </>
                    ) : (
                ...
                <div className="h-auto md:h-24 bg-gray-800 border-t border-gray-700 p-4 flex items-center justify-between px-4 md:px-8 shrink-0">
                    ...
                    <div className="text-center hidden md:block">
                        <div className="text-pageant-gold font-bold text-sm">CONTROLLER</div>
                    </div>

                    <button
                        onClick={() => setIndex(Math.min(candidates.length - 1, currentIndex + 1))}
